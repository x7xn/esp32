...
/**************************soft crc************************/
//crc32
static const uint32_t crc32_table_0xEDB88320[] = {
...
uint32_t soft_crc32(const void *pdata,int data_size,uint32_t crc)
{
	...
	while (data_size--) {
		...
		result >>= 8;
		result ^= crc32_table_0xEDB88320[crc_H8 ^ (*data)];
		data++;
	}
...
***************************************************/
const uint8_t default_crc8_table[] = {
 ...
 */
uint8_t soft_crc8(const uint8_t crc8_table[256], uint8_t *pdata, size_t nbytes, uint8_t crc)
{
	const uint8_t *table = crc8_table ? crc8_table : default_crc8_table;
